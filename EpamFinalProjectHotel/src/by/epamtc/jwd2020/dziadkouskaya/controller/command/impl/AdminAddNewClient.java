...
public class AdminAddNewClient implements Command {
	...
	public static final String PATH_TO_PERSONAL_DATA_PAGE = "/WEB-INF/jspPages/admin_new_client_adding.jsp";
	public static final String PATH_TO_WELCOME_PAGE = "mainPage?command=admin_welcome_new_client";
	public static final String PATH_TO_ERROR_PAGE = "mainPage?command=error_page";

	private static final Logger logger = LogManager.getLogger(AdminAddNewClient.class);
		...
		try {
			...
			if (!passwordCheck) {
				...
				if (checkingResult == null || !checkingResult.equals(STRING_FROM_DAO_IF_USER_WAS_NOT_FOUND_IN_DB)) {
					...
					request.getSession(true).setAttribute("client_code", id);
			...
			logger.error("AdminAddNewClient ServiceException", e);
			response.sendRedirect(PATH_TO_ERROR_PAGE);

		} catch (Exception e) {
			logger.error("AdminAddNewClient Exception", e);
			response.sendRedirect(PATH_TO_ERROR_PAGE);
		}
	}
